...
SPIRITUAL_CLAIM_AS_FACT_PATTERNS: tuple[str, ...] = (
    ...
    # Astrology as prediction, doctrine astrology-specific rule
    r"\byour (?:horoscope|chart|placement) (?:says?|shows?|means) you will\b",
    r"\b(?:mercury retrograde|your sign) means (?:this|your) \w+ will\b",
    (
        r"\byou(?:'re| are) (?:definitely |truly )?(?:compatible|incompatible) with "
        r"(?:them|him|her) because of (?:your|their) sign\b"
    ),
    ...
    # Manifestation as causal blame, doctrine manifestation-specific rule
    (
        r"\byou(?:'re| are) (?:struggling|dealing) with this because your "
        r"(?:energy|vibration|mindset) (?:is not|isn'?t) aligned\b"
    ),
    r"\byou didn'?t manifest .+ because you didn'?t believe\b",
    (
    ...
    # harm-exception rule
    (
        r"\bthis (?:karmic relationship|soul contract) means you (?:need|have) to "
        r"(?:stay|endure|accept)\b"
    ),
    (
        r"\byou(?:'re| are) (?:meant|destined) to (?:go through|endure) this "
        r"(?:with them|together)\b"
    ),
