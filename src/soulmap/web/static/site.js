  ...
  Alpine.data("skillCatalog", () => ({
    ...
    renderSkillResults(ranked, grid, questionResults) {
      ...
      const cardsBySlug = new Map(
        cards.map((card) => [card.dataset.skillSlug || card.querySelector(".code-pill")?.textContent?.trim(), card])
      );
      ...
      for (const card of cards) {
        const slug = card.dataset.skillSlug || card.querySelector(".code-pill")?.textContent?.trim();
