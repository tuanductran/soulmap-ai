...
Examples
--------
python -m soulmap_devtools.cli.build_skill                  # zip build
python -m soulmap_devtools.cli.build_skill --skill          # .skill build

Formats
-------
soulmap-ai.zip
    Standard distribution zip for manual extraction and document-based tools.
    Includes root SKILL.md, AGENTS.md, LICENSE, skills/, and templates/.
    Excludes .claude-plugin/.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m soulmap_devtools.cli.build_skill",
        description="Build SoulMap AI distribution artifacts.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent("""\
            examples:
              python -m soulmap_devtools.cli.build_skill           build the standard zip
              python -m soulmap_devtools.cli.build_skill --skill   build the .skill package
        """),
    )
    parser.add_argument(
        "--skill",
