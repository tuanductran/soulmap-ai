...
def _skill_cards(locale: str, query: str = "") -> str:
    ...
    for entry in CATALOG:
        ...
        cards.append(
            ...
            f'<div class="skill-card__actions"><a class="button small" href="{escape(detail_href, quote=True)}" aria-haspopup="dialog" aria-controls="skill-modal" data-detail-url="{escape(partial_href, quote=True)}" x-on:click.prevent="open(\'{escape(entry.slug)}\', $event.currentTarget, $event.currentTarget.dataset.detailUrl)">{_text(locale, "details")}</a><a class="link-button small secondary" href="{escape(raw_path(entry.slug, locale), quote=True)}" target="_blank" rel="noopener">{_text(locale, "raw")}</a></div>'
            "</article>"
        )
    return (
        "".join(cards)
