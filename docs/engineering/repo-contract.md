...
| `site/` | Static website generated from the shipped knowledge package | Local-only; never packaged in `dist/soulmap-ai.zip` or `dist/soulmap-ai.skill` | TypeScript generator, page templates, design tokens, and the publish-boundary tests; no authored product claims | `npm run check` in `site/` (typecheck, unit tests, build, and generated-site validation) as its own CI job; see [ADR 0005](adr/0005-generated-static-site.md) |
