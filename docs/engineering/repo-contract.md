...
| `website/` | Public website presentation assets | Published in the repository, not packaged in either dist archive | Jinja2 templates, stylesheet, minimal progressive-enhancement JavaScript, and self-hosted fonts. No content: every page is generated from canonical files by `src/soulmap/devtools/web/` | `uv run soulmap build-site --check`, the website tests in `tests/web/`, and the standard Markdown and link gates |
