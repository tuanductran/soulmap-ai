...
| @playwright/test | `web/pnpm-lock.yaml` | [Playwright Test documentation](https://playwright.dev/docs/intro) | Node-based browser regression for the independent React static workspace. Its browser binaries are not Python package dependencies; the Pages workflow installs the selected Chromium, Firefox and WebKit engines explicitly. |
