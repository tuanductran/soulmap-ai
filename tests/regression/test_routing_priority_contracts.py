...
def test_stage_one_second_message_forces_mirror(monkeypatch: pytest.MonkeyPatch) -> None:
    _install(monkeypatch)
    monkeypatch.setattr(
        framework_selector,
        "detect_existential",
...
def test_high_intensity_has_no_secondary_layer(monkeypatch: pytest.MonkeyPatch) -> None:
    _install(monkeypatch)
    monkeypatch.setattr(
        framework_selector, "detect_intensity", lambda *args, **kwargs: {"level": "HIGH"}
    )
    monkeypatch.setattr(
        framework_selector, "detect_anger", lambda *args, **kwargs: {"anger_detected": True}
    )
    monkeypatch.setattr(
        framework_selector, "detect_bypass", lambda *args, **kwargs: {"bypass_detected": True}
    )
    monkeypatch.setattr(
        framework_selector, "detect_somatic", lambda *args, **kwargs: {"somatic_detected": True}
    )
...
def test_high_intensity_outranks_insight(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = framework_selector.select_framework("I see everything clearly but I am overwhelmed.", [])
    assert result["primary_framework"] == "DE_ESCALATION"
    assert result["secondary_layer"] is None
