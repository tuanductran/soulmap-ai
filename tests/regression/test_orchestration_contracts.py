...
def _install_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    for name, value in _DETECTOR_DEFAULTS.items():
        monkeypatch.setattr(
            framework_selector,
            name,
...
def test_stage_one_first_message_forces_mirror_even_with_framework_signal(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Stage 1 must not jump into a specialist framework on the first turn."""
...
def test_stage_one_second_message_forces_mirror_even_with_framework_signal(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Stage 1 must still hold Mirror on the second user message."""
    _install_defaults(monkeypatch)
    monkeypatch.setattr(
        framework_selector,
        "detect_existential",
...
def test_high_intensity_never_attaches_secondary_layer(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """HIGH intensity is a hard De-escalation override with no secondary layer."""
    _install_defaults(monkeypatch)
    monkeypatch.setattr(
        framework_selector,
        "detect_intensity",
...
def test_breakthrough_insight_outranks_competing_grief_signal(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A detected breakthrough routes to Meaning Integration before topic routes."""
    _install_defaults(monkeypatch)
    monkeypatch.setattr(
        framework_selector,
        "detect_insight",
