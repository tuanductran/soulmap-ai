...
async function expectNoA11yViolations(findings: A11yFinding[], testInfo: TestInfo) {
  await testInfo.attach("axe-audit-findings", { body: JSON.stringify(findings, null, 2), contentType: "application/json" });
  expect(findings, findings.map((finding) => `${finding.state}: ${finding.rule}`).join("; ")).toEqual([]);
}

test("a11y audit: every public route in every locale has no automatically detectable WCAG A/AA violations", async ({ page }, testInfo) => {
  ...
  for (const locale of locales) {
    for (const route of routes) {
      ...
      await test.step(`${locale || "en"}/${route || "home"}`, async () => {
        ...
        findings.push(...await collectA11yViolations(page, `${locale || "en"}-${route || "home"}`));
      });
    }
  }
...
test("a11y audit: menus, disclosure and provider handoff retain WCAG A/AA conformance when open", async ({ page }, testInfo) => {
  const findings: A11yFinding[] = [];
...
test("a11y audit: mobile navigation scroll cue is labelled and leaves a conformant page state", async ({ page }, testInfo) => {
  ...
  await expectNoA11yViolations(await collectA11yViolations(page, "vi-mobile-navigation-cue"), testInfo);
});
