...
function sanitize(markdown, labels) {
  return markdown
    ...
    .replace(
      /(?<!\w)(?:\.claude\/|\.github\/|src\/|tests\/|pyproject\.toml|uv\.lock)(?:[A-Za-z0-9_./-]*)/g,
...
async function rawMarkdown(entry, locale) {
  ...
  let names = [];
  try {
    names = (await readdir(skillsDir)).filter((name) => name.endsWith(".md")).sort();
  } catch {
    /* missing source renders a clear bundle message */
  }
  const parts = [
    `# ${labels.bundle_title}: ${fields.title}\n\n`,
...
for (const locale of ["en", "vi", "ko"]) {
  ...
  for (const entry of catalog.entries)
    await writeFile(
      path.join(target, `${entry.slug}.md`),
      await rawMarkdown(entry, locale),
      "utf8",
    );
}
