      ...
      <div className="grid gap-10 lg:grid-cols-[0.72fr_1.6fr] lg:gap-20">
        <aside className="lg:sticky lg:top-8 lg:self-start">
          <p className="text-xs font-extrabold uppercase tracking-[0.2em] text-[#1d5f58]">
            {t("common.coordinates")} / 06
          </p>
          <h1 className="mt-4 max-w-md font-serif text-5xl leading-[0.93] tracking-[-0.06em] text-[#122b2c] sm:text-6xl">
            {t("skills.title")}
          </h1>
          ...
          <div className="relative mt-8">
            ...
            <input
              ...
              className="h-13 w-full rounded-2xl border border-[#cfd9d0] bg-white/75 pl-11 pr-4 text-sm outline-none transition placeholder:text-[#85938e] focus:border-[#267b72] focus:ring-4 focus:ring-[#267b72]/10"
            />
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {filtered.map((skill, index) => {
            ...
            return (
              <article
                key={skill.slug}
                className="group relative flex min-h-72 flex-col overflow-hidden rounded-[1.5rem] border border-[#dfe5dc] bg-[#fcfbf7] p-6 shadow-[0_12px_30px_rgba(20,49,45,0.05)] transition duration-200 hover:-translate-y-1 hover:shadow-[0_22px_44px_rgba(20,49,45,0.11)]"
              >
                <div className="flex items-start justify-between gap-3">
                  <span
                    className={`rounded-full px-3 py-1 text-[0.68rem] font-extrabold uppercase tracking-[0.16em] ${accentClass[skill.accent]}`}
                  >
                    {copy.group}
                  </span>
                  <span className="font-serif text-sm text-[#5d756b]">0{index + 1}</span>
                </div>
                <h2 className="mt-6 font-serif text-3xl leading-[0.98] tracking-[-0.045em] text-[#163736]">
                  {copy.title}
                </h2>
                ...
                <button
                  ...
                  className="mt-auto inline-flex items-center gap-2 pt-6 text-sm font-extrabold text-[#1e655e] transition group-hover:gap-3"
                >
          ...
          {!filtered.length && (
            <div className="rounded-[1.5rem] border border-dashed border-[#bdcbc0] p-8 text-sm leading-6 text-[#5a6d68]">
              {t("skills.noResults")}
            </div>
          )}
        </div>
