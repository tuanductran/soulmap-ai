...
const accentClass: Record<string, string> = { Moss: "bg-[#dcebe2] text-[#1f655e]", Clay: "bg-[#f1e0d2] text-[#7f3e29]", Ink: "bg-[#dce5e6] text-[#234a50]" };
    ...
    <section className="mx-auto max-w-7xl px-5 py-14 sm:px-8 lg:px-12 lg:py-20">
      <div className="grid gap-10 lg:grid-cols-[0.72fr_1.6fr] lg:gap-20">
        <aside className="lg:sticky lg:top-8 lg:self-start">
          <p className="text-xs font-extrabold uppercase tracking-[0.2em] text-[#1d5f58]">{t("common.coordinates")} / 06</p>
          ...
          <p className="mt-3 text-xs font-bold uppercase tracking-[0.14em] text-[#4e665e]">{filtered.length} {t("skills.count")}</p>
        </aside>
        <div className="grid gap-4 sm:grid-cols-2">
          {filtered.map((skill, index) => {
            ...
            return <article key={skill.slug} className="group relative flex min-h-72 flex-col overflow-hidden rounded-[1.5rem] border border-[#dfe5dc] bg-[#fcfbf7] p-6 shadow-[0_12px_30px_rgba(20,49,45,0.05)] transition duration-200 hover:-translate-y-1 hover:shadow-[0_22px_44px_rgba(20,49,45,0.11)]">
              <div className="flex items-start justify-between gap-3"><span className={`rounded-full px-3 py-1 text-[0.68rem] font-extrabold uppercase tracking-[0.16em] ${accentClass[skill.accent]}`}>{copy.group}</span><span className="font-serif text-sm text-[#5d756b]">0{index + 1}</span></div>
