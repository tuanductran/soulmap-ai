  ...
  return (
    <div className="min-h-screen overflow-x-clip bg-[#f7f6ef] text-[#122b2c]">
      <header className="relative z-30 border-b border-[#dfe5dc] bg-[#f7f6ef]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8 lg:px-12">
          ...
          <Menu as="div" className="relative">
            <MenuButton className="inline-flex items-center gap-2 rounded-full border border-[#d8dfd8] bg-white/70 px-4 py-2 text-xs font-bold tracking-[0.08em] text-[#234d4a] transition hover:bg-white">
                ...
                <MenuItem key={item}>
                  {({ focus }) => (
                    <Link
                      ...
                      className={`block rounded-xl px-3 py-2 text-sm font-semibold ${focus ? "bg-[#e5efe8] text-[#1c625b]" : "text-[#284543]"}`}
                    >
                      {labels[item]}{" "}
        ...
        <div className="relative mx-auto max-w-7xl">
          <nav
            ...
            className="overflow-x-auto px-5 pb-3 pr-14 [scrollbar-width:thin] sm:px-8 sm:pr-8 lg:px-12"
            ...
            <div className="flex min-w-max gap-1">
              {links.map((link) => (
                <Link
                  key={link.key}
                  to={link.to}
                  className="rounded-full px-4 py-2 text-sm font-medium text-[#526565] transition hover:bg-[#e6eee8] hover:text-[#1b5550] active:scale-[0.98]"
                  activeProps={{ className: "bg-[#dcebe2] text-[#1b5550]" }}
                >
                  {t(`nav.${link.key}`)}
                </Link>
              ))}
              <Menu as="div" className="relative">
                <MenuButton className="inline-flex items-center gap-1 rounded-full px-4 py-2 text-sm font-medium text-[#526565] transition hover:bg-[#e6eee8] hover:text-[#1b5550]">
                  {t("nav.more")} <ChevronDown className="size-3" />
                </MenuButton>
                <MenuItems
                  anchor="bottom start"
                  portal
                  className="z-40 w-44 origin-top-left rounded-2xl border border-[#d8dfd8] bg-[#fbfaf5] p-1.5 shadow-xl [--anchor-gap:8px] focus:outline-none"
                >
                  {moreLinks.map((link) => (
          ...
          {canScrollNavigation && (
            <button
              ...
              className="absolute right-2 top-0 inline-flex size-9 items-center justify-center rounded-full border border-[#cfd9d0] bg-[#f7f6ef]/95 text-[#1e655e] shadow-sm backdrop-blur sm:hidden"
      ...
      <footer className="border-t border-[#dfe5dc] bg-[#f2f1e8] px-5 py-8 sm:px-8 lg:px-12">
        <div className="mx-auto flex max-w-7xl flex-col justify-between gap-4 text-sm text-[#5d6c67] sm:flex-row sm:items-center">
          <p>{t("footer")}</p>
          <a
