...
type ProviderDialogProps = { open: boolean; skill: Skill | null; locale: Locale; onClose: () => void };
      ...
      <div className="fixed inset-0 grid place-items-end p-3 sm:place-items-center sm:p-6">
        <DialogPanel className="w-full max-w-xl rounded-[1.75rem] border border-white/70 bg-[#fbfaf5] p-6 shadow-[0_28px_80px_rgba(18,43,44,0.28)] sm:p-8">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            ...
            <a href={rawBundleUrl(skill, locale)} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-[#bfcfc4] px-5 text-sm font-bold text-[#1e5550] transition hover:bg-[#eaf0eb]">{t("skills.raw")}<ArrowUpRight className="size-4" /></a>
          </div>
          <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-[#526565]">
            <a href="https://claude.ai/new" target="_blank" rel="noreferrer" className="rounded-full border border-[#d8dfd8] px-3 py-2 hover:bg-white">Claude <ArrowUpRight className="ml-1 inline size-3" /></a>
            <a href="https://chatgpt.com/" target="_blank" rel="noreferrer" className="rounded-full border border-[#d8dfd8] px-3 py-2 hover:bg-white">ChatGPT <ArrowUpRight className="ml-1 inline size-3" /></a>
          </div>
        </DialogPanel>
      </div>
    </Dialog>
