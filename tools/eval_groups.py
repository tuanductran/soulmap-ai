...
def _load_groups() -> list[GroupEntry]:
    path = REPO_ROOT / "evals" / "groups.json"
    ...
    *,
    ...
    for group in groups:
        ...
        for item in group["items"]:
            ...
            gate = apply_safety_gate(message, history, {}, selection)
            ...
            if expected_safety_status is not None:
                assertion_checks += 1
                item_has_expectation = True
                checks.append(gate["status"] == expected_safety_status)
            if expected_safety_reason is not None:
                assertion_checks += 1
                item_has_expectation = True
                checks.append(gate["reason"] == expected_safety_reason)
            ...
            results.append(
                {
                    ...
                    "actual_safety_status": gate["status"],
                    "actual_safety_reason": gate["reason"],
                    "ok": passed,
                }
            )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run framework-routing QA checks from evals/groups.json."
    )
