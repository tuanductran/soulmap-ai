...
Use this skill when reviewing or improving Python implementation quality in `modules/`,
`tools/`, `tests/`, or `scripts/`.
...
- `pyproject.toml`
- `.claude/rules/python-tooling.md`
- `.claude/rules/source-character-safety.md`
- `docs/DEV.md`
...
- local code that should use shared helpers in `modules/cli_payload.py` or
  `modules/text_normalization.py`

## Workflow

1. Read the target Python files and the closest tests first.
2. Compare the implementation with `pyproject.toml` and `.claude/rules/python-tooling.md`.
