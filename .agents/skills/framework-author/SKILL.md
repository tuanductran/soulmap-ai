...
1. Create or update a detector in `src/soulmap_runtime/detectors/your_framework_detector.py`
2. Add it to the framework selector in `src/soulmap_runtime/routing/framework_selector.py`
3. Reference it from `detector-engineer` skill if creating new detector patterns

### Add test coverage

Create eval test cases in `evals/datasets/groups.json`:
...
8. Run `python -m soulmap_devtools.cli.build_skill` and `python -m pytest -n auto -q` before committing.
...
- A corresponding detector exists in `src/soulmap_runtime/detectors/` or is planned
- At least one eval group in `evals/datasets/groups.json` covers the primary activation signal
- `python -m soulmap_devtools.cli.build_skill` includes the new file without error
