...
6. Run `python -m soulmap_devtools.cli.eval_groups` and verify no existing assertions broke.
7. Run `python -m pytest -n auto -q` before committing.
...
- `python -m soulmap_devtools.cli.eval_groups` passes for the new group
